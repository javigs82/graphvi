...
import es.javier.util.graph.visitor.Visitor;
import es.javier.util.graph.visitor.VisitorEX;
...
public class Graph<T> {
	...
	public Graph(ArrayList<Vertex<T>> verticies, ArrayList<Edge<T>> edges) {
	...
	public void javiSpanningTree(Vertex<T> v, DFSVisitor<T> visitor) {
		v.visit();
		if (visitor != null)
			visitor.visit(this, v);// do exactly the same as v.visit();
		...
		for (int i = 0; i < v.getOutgoingEdgeCount(); i++) {
			...
			if (!e.isMarked()) {
				...
				e.mark();
				javiSpanningTree(e.getTo(), visitor);
			}
		}
	}
